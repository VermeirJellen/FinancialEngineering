% BLSBARRIERDELTA - Compute Deltas for exotic barrier options 
...
%
% [Delta] = blsBarrierDelta(sigma,S0,Strike,Barrier,Rebate,Time,Rate)
% [Delta] = blsBarrierDelta(sigma,S0,Strike,Barrier,Rebate,Time,Rate,Yield,Type)
%
% Inputs:
...
%
% Type - The barrier type for which the Delta will be
...
%   The stock does not pay dividends. We would like to Delta a DOBC option
%   with time until maturity of 1.5 years.
%   The strike Delta of the option is 55$, the barrier level is placed 
%   at 40$ and the rebate is 2$. The risk free rate is 5% per annum. 
%	The following command will  return the Delta of this DOBC barrier option:
% 
% 	Delta = blsBarrierDelta(0.20, 50, 55, 40, 2, 1.5, 0.05, 0,'dobc')
%
% Example 2:
%	Consider a more involved example where we Delta 9 different "in" type
...
%	Deltas = blsBarrierDelta(sigma,S0,K,B,R,T,r,q,type);
%	
% Notes:
...
%
% See also: BLSBARRIER, BLSBARRIERGAMMA, BLSBARRIERVEGA, BLSBARRIERTHETA, 
...
function [Delta] = blsBarrierDelta(sigma,St,K,B,R,T,r,q,type)

if(nargin < 7)
	error('Not enough input parameters. Type "help blsBarrierDelta" for more information');
end
...
if(not(isempty(find(ismember(char(type),barrierTypes)==0))))
	error('Unknown option type input argument detected: Type "help blsBarrierDelta" for more information');
end;
...
dS = 0.0001;
Delta = (blsBarrier(sigma,St+dS,K,B,R,T,r,q,type) - blsBarrier(sigma,St-dS,K,B,R,T,r,q,type))./(2*dS);