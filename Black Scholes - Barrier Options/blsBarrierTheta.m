% Compute Thetas for exotic barrier options under the
% Black-Scholes model.
%
...
%
% [Theta] = blsBarrierTheta(sigma,S0,Strike,Barrier,Rebate,Time,Rate)
% [Theta] = blsBarrierTheta(sigma,S0,Strike,Barrier,Rebate,Time,Rate,Yield,Type)
%
% Inputs:
...
%
% Type - The barrier type for which the Theta will be
...
%
% Output:
%   Theta - Black scholes sensitivity to time until maturity change
%
% Example 1:
%   Consider a stock trading at 50$ with a Black-Scholes volatility of 20%. 
%   The stock does not pay dividends. We would like to Theta a DOBC option
%   with time until maturity of 1.5 years.
%   The strike Theta of the option is 55$, the barrier level is placed 
%   at 40$ and the rebate is 2$. The risk free rate is 5% per annum. 
%	The following command will  return the Theta of this DOBC barrier option:
% 
% 	Theta = blsBarrierTheta(0.20, 50, 55, 40, 2, 1.5, 0.05, 0,'dobc')
%
% Example 2:
%	Consider a more involved example where we Theta 9 different "in" type
...
%	Thetas = blsBarrierTheta(sigma,S0,K,B,R,T,r,q,type);
%	
% Notes:
...
%
%
function [Theta] = blsBarrierTheta(sigma,St,K,B,R,T,r,q,type)

if(nargin < 7)
	error('Not enough input parameters. Type "help blsBarrierTheta" for more information');
end
...
dT = 0.0001;
Theta = (blsBarrier(sigma,St,K,B,R,T+dT,r,q,type) - blsBarrier(sigma,St,K,B,R,T,r,q,type))./(dT);