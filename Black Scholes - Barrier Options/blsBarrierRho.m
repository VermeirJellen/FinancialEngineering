% BLSBARRIERRHO - Compute Rhos for exotic barrier options under the
% Black-Scholes model.
%
...
%
% [Rho] = blsBarrierRho(sigma,S0,Strike,Barrier,Rebate,Time,Rate)
% [Rho] = blsBarrierRho(sigma,S0,Strike,Barrier,Rebate,Time,Rate,Yield,Type)
%
% Inputs:
%   Sigma - Black Scholes volatility of the underlying asset
%
%   S0 - Current stock Rho of the underlying asset
%
%   Strike - Strike (i.e, exercise) Rho of the option
%
%   Barrier - Barrier Rho of the option
%
...
%
% Type - The barrier type for which the Rho will be
...
%   The stock does not pay dividends. We would like to Rho a DOBC option
%   with time until maturity of 1.5 years.
%   The strike Rho of the option is 55$, the barrier level is placed 
%   at 40$ and the rebate is 2$. The risk free rate is 5% per annum. 
%	The following command will  return the Rho of this DOBC barrier option:
% 
% 	Rho = blsBarrierRho(0.20, 50, 55, 40, 2, 1.5, 0.05, 0,'dobc')
%
% Example 2:
%	Consider a more involved example where we Rho 9 different "in" type
...
%	Rhos = blsBarrierRho(sigma,S0,K,B,R,T,r,q,type);
%	
% Notes:
...
% BLSBARRIERTHETA, BLSBARRIERIMPV
function [Rho] = blsBarrierRho(sigma,St,K,B,R,T,r,q,type)

if(nargin < 7)
	error('Not enough input parameters. Type "help blsBarrierRho" for more information');
end
...
if(not(isempty(find(ismember(char(type),barrierTypes)==0))))
	error('Unknown option type input argument detected: Type "help blsBarrierRho" for more information');
end;

dT = 0.0001;
Rho = (blsBarrier(sigma,St,K,B,R,T+dT,r,q,type) - blsBarrier(sigma,St,K,B,R,T,r,q,type))./(dT);