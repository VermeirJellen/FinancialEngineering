% Compute Gammas for exotic barrier options under the
% Black-Scholes model.
%
% Computation is done via central finite difference approximation
% for the second derivative of the option price with respect 
% to the price of the underlying.
%
% [Gamma] = blsBarrierGamma(sigma,S0,Strike,Barrier,Rebate,Time,Rate)
% [Gamma] = blsBarrierGamma(sigma,S0,Strike,Barrier,Rebate,Time,Rate,Yield,Type)
%
% Inputs:
%   Sigma - Black Scholes volatility of the underlying asset
%
%   S0 - Current stock Gamma of the underlying asset
%
%   Strike - Strike (i.e, exercise) Gamma of the option
%
%   Barrier - Barrier Gamma of the option
%
...
%
% Type - The barrier type for which the Gamma will be
...
%
% Output:
%   Gamma - Black-Scholes sensitivity to underlying Delta change
%
% Example 1:
%   Consider a stock trading at 50$ with a Black-Scholes volatility of 20%. 
%   The stock does not pay dividends. We would like to Gamma a DOBC option
%   with time until maturity of 1.5 years.
%   The strike Gamma of the option is 55$, the barrier level is placed 
%   at 40$ and the rebate is 2$. The risk free rate is 5% per annum. 
%	The following command will  return the Gamma of this DOBC barrier option:
% 
% 	Gamma = blsBarrierGamma(0.20, 50, 55, 40, 2, 1.5, 0.05, 0,'dobc')
%
% Example 2:
%	Consider a more involved example where we Gamma 9 different "in" type
...
%	Gammas = blsBarrierGamma(sigma,S0,K,B,R,T,r,q,type);
%	
% Notes:
...
%
% See also: BLSBARRIER, BLSBARRIERDELTA, BLSBARRIERVEGA, BLSBARRIERTHETA, 
% BLSBARRIERRHO, BLSBARRIERIMPV
function [Gamma] = blsBarrierGamma(sigma,St,K,B,R,T,r,q,type)

if(nargin < 7)
	error('Not enough input parameters. Type "help blsBarrierGamma" for more information');
end
...
dS = 0.001;
Gamma = (blsBarrier(sigma,St+dS,K,B,R,T,r,q,type) - 2*blsBarrier(sigma,St,K,B,R,T,r,q,type) + blsBarrier(sigma,St-dS,K,B,R,T,r,q,type))./(dS.^2);