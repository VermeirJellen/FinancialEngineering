% BLSLOOKBACKTHETA - Compute the Theta for exotic lookback options 
...
%
% [Theta] = blsLookbackTheta(sigma,St,MinMax,Time,Rate)
% [Theta] = blsLookbackTheta(sigma,St,MinMax,Time,Rate,Yield,Type)
%
% Inputs:
...
%	The following command will return the Theta of this lookback option:
% 
% 	Theta = blsLookbackTheta(0.20, 50, 48, 1.5, 0.05, 0, 0)
%
% Example 2:
...
%	The Theta's can be obtained by giving vectors or matrices as input. For example:
%	S0 = [100 110 120; 100 110 120];
%	M = [95 105 120; 105 110 128];
%	sigma = [0.2 0.1 0.15; 0.30 0.20 0.10];
%	q = [0 0.05 0.02; 0 0 0];
%	type = [0 0 0; 1 1 1];
%	Thetas = blsLookbackTheta(sigma,S0,M,0.5,0.1,q,type);
%	
% Notes:
...
function [Theta] = blsLookbackTheta(sigma,St,M,T,r,q,type)

if(nargin < 5)
	error('Not enough input parameters. Type "help blsLookbackTheta" for more information');
end;
...
if(not(isempty(find(ismember(type,[0 1])==0))))
	error('Unknown option type input argument detected: Type "help blsLookbackTheta" for more information');
end;

dT = 0.0001;
Theta = (blsLookback(sigma,St,M,T+dT,r,q,type) - blsLookback(sigma,St,M,T,r,q,type))./(dT);