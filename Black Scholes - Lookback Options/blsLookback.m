% BLSLOOKBACK - Compute prices for exotic lookback options 
% under the Black-Scholes model.
% 
% [price] = blsLookback(sigma,St,MinMax,Time,Rate)
% [price] = blsLookback(sigma,St,MinMax,Time,Rate,Yield,Type)
%
% Inputs:
...
%
% Output:
%   price - The price of the lookback option
%
% Example 1:
...
%	The following command will return the price of this lookback option:
% 
% 	price = blsLookback(0.20, 50, 48, 1.5, 0.05, 0, 0)
%
% Example 2:
...
%	The options can be priced in bulk by giving vectors or matrices as input. For example:
%	S0 = [100 110 120; 100 110 120];
%	M = [95 105 120; 105 110 128];
%	sigma = [0.2 0.1 0.15; 0.30 0.20 0.10];
%	q = [0 0.05 0.02; 0 0 0];
%	type = [0 0 0; 1 1 1];
%	prices = blsLookback(sigma,S0,M,0.5,0.1,q,type);
%	
% Notes:
...
%
% References: 
%   (1) Research program in Finance, working paper series (NO. 220)
%	    Exotic options, by Mark Rubinstein
%	(2) Journal of Finance, December 1979
%		Path Dependend Options: Buy at the Low, Sell at the High,
%		by Barry Goldman, Howard Sosin and Mary Ann Gato
%
% See also: BLSLOOKBACKDELTA, BLSLOOKBACKGAMMA, BLSLOOKBACKVEGA, BLSLOOKBACKTHETA, 
% BLSLOOKBACKRHO, BLSLOOKBACKIMPV
function [price] = blsLookback(sigma,St,M,T,r,q,type,returnGreeks)

if(nargin < 5)
	error('Not enough input parameters. Type "help blsLookback" for more information');
end;
...
if(nargin < 8 || isempty(returnGreeks))
	returnGreeks=0;
end;
if(not(isempty(find(ismember(type,[0 1])==0))))
	error('Unknown option type input argument detected: Type "help blsLookback" for more information');
end;

phi = ones(size(type));
phi(ismember(type,1)) = -1;
...
b = log(St./M);
mu = log(r1./q1) - (1/2)*sigma.^2;
lambda = (1/2)*sigma.^2./log(r1./q1);

P1 = (b+mu.*t)./(sigma.*sqrt(t));
P2 = (-b+mu.*t)./(sigma.*sqrt(t));
P3 = (-b-mu.*t-sigma.^2.*t)./(sigma.*sqrt(t));
price = phi.*(St.*q1.^(-t) - M.*r1.^(-t).*normcdf(phi.*P1) + M.*r1.^(-t).*lambda.*exp(b.*(1-(1./lambda))).*normcdf(phi.*P2) ...
	- St.*q1.^(-t).*(1+lambda).*normcdf(phi.*P3));