% BLSLOOKBACKGAMMA - Compute the gamma for exotic lookback options 
...
%
% [gamma] = blsLookbackGamma(sigma,St,MinMax,Time,Rate)
% [gamma] = blsLookbackGamma(sigma,St,MinMax,Time,Rate,Yield,Type)
%
% Inputs:
...
%	The following command will return the gamma of this lookback option:
% 
% 	gamma = blsLookbackGamma(0.20, 50, 48, 1.5, 0.05, 0, 0)
%
% Example 2:
...
%	The gamma's can be obtained by giving vectors or matrices as input. For example:
%	S0 = [100 110 120; 100 110 120];
%	M = [95 105 120; 105 110 128];
%	sigma = [0.2 0.1 0.15; 0.30 0.20 0.10];
%	q = [0 0.05 0.02; 0 0 0];
%	type = [0 0 0; 1 1 1];
%	gammas = blsLookbackGamma(sigma,S0,M,0.5,0.1,q,type);
%	
% Notes:
...
%
% See also: BLSLOOKBACK, BLSLOOKBACKDELTA, BLSLOOKBACKVEGA, BLSLOOKBACKTHETA, 
...
function [gamma] = blsLookbackGamma(sigma,St,M,T,r,q,type)

if(nargin < 5)
	error('Not enough input parameters. Type "help blsLookbackGamma" for more information');
end;
...
if(not(isempty(find(ismember(type,[0 1])==0))))
	error('Unknown option type input argument detected: Type "help blsLookbackGamma" for more information');
end;

dS = 0.001;
gamma = (blsLookback(sigma,St+dS,M,T,r,q,type) - 2*blsLookback(sigma,St,M,T,r,q,type) + blsLookback(sigma,St-dS,M,T,r,q,type))./(dS.^2);